    ...
    try {
      ...
      if (fullProducto) {
        agregarAlCarrito({
          ...
          stock: fullProducto.stock // ✅ CLAVE: AHORA SÍ ESTÁ
        });
      ...
      <section className="pt-32 pb-16 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-serif mb-4 text-white">Nuestras <span className="text-[#EC4899]">Pulseras</span></h1>
            <p className="text-gray-400 max-w-2xl mx-auto font-light">Descubre nuestra colección de pulseras artesanales, detalles que envuelven tu estilo con delicadeza y sutileza.</p>
          </div>
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8">
              {productos.map((producto) => (
                <Link key={producto.id} href={`/producto/${producto.id}`} className="group bg-[#2D2D2D] p-4 rounded-xl border border-[#F59E0B]/30 hover:border-[#EC4899] transition-all duration-300 block">
          ...
          <div className="mt-12 text-center">
            <Link href="/tienda" className="text-[#EC4899] hover:text-[#F59E0B] transition-colors">← Volver a todas las categorías</Link>
          </div>
        </div>
      </section>
      <Footer />
